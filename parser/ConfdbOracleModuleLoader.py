...
#                                     db=dbname )
        ...
        cursor.execute("SELECT distinct paramtype from u_moelements")
...
#	thecursor.execute("SELECT superId FROM SuperIds ORDER BY superId DESC");
	...
	print "SELECT u_moduletemplates.id, u_moduletemplates.cvstag from u_modt2rele,  u_moduletemplates where u_modt2rele.id_release=" + str(self.comparetorel
id) + " and u_moduletemplates.id= u_modt2rele.id_modtemplate and u_moduletemplates.name= '" + modclassname + "'"
	else:
